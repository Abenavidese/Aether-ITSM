...
| :-- | :--- | :--- | :--- |
| A | Low-risk ticket reaches a risky tool (e.g. `modify_iam_access` via Spanish wording the floor missed) | `src/agent/tool_policy.py`: default-deny policy per tool (risk, params, validators); a tool riskier than the ticket never runs — instead the ticket's risk is raised (once) and it goes back through Policy, and risk-3 tools only run after human approval. Identity params are never shown to the model. Risk floors in EN+ES and from the classifier's `tools_required`. | Implemented |
