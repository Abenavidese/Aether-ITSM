...
async def vercel_log_drain(tenant_id: str, request: Request):
    ...
    try:
        ...
        secret = decrypt_token(company.vercel_drain_secret) if company and company.vercel_drain_secret else None
    finally:
        db.close()
    ...
    stored = ingest(tenant_id, parse_payload(raw_body), project_ids)
